...
|---|---|
| el que trae el diagnóstico `R50.5` | ese código **no existe** en el catálogo CIE-10 |
...
**Por qué solo 2 de 9, y qué haría falta:**

| Lo que falta | Cuántos casos desbloquea |
|---|---|
| El **catálogo real de diagnósticos** (`lpdiagnosticos` de ASTGU) | 4 — sin él no podemos afirmar que un código no existe ni comparar el nombre del diagnóstico |
...
1. **Los catálogos reales** de la BD ASTGU: diagnósticos (`lpdiagnosticos`) y el histórico
   (`lpausentismos`). Hoy usamos catálogos de prueba armados a mano.
...
- **Apunta a catálogos de prueba, no a la BD ASTGU real.** Es lo primero que cambia al instalar.
- **La detección de adulteración está a medias**, por lo de arriba: las señales están
  especificadas y medidas, pero tres de las cinco familias necesitan datos que no tenemos.
